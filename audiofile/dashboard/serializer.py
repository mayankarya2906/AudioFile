...
def pop_out_extra_fields(request_data, request_keys, type_of_audio):
    ...
    mapping_of_fields = {
        'audio_book': ['title_of_the_audiobook', 'author_of_audiobook',
                       'narrator', 'duration', 'upload_time'],

        'song': ['name_of_song', 'duration', 'upload_time'],

        'podcast': ['name_of_podcast', 'host', 'participants', 'duration',
                    'upload_time']
        }
    # find the field list
    fields = mapping_of_fields.get(type_of_audio)

    # pop out the extra fields from
    # the request data and return the relevant data
    ...
    if upload_time is not None:
        ...
        if upload_time < django.utils.timezone.now():
            raise serializers.ValidationError(
              {"error": "upload time cannot be in past"})
...
class AudioBookSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        audio_book_data = pop_out_extra_fields(
            request_data=audio_book_data,
            request_keys=audio_book_data_keys,
...
class PodcastSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        podcast_data = pop_out_extra_fields(
            request_data=podcast_data,
            request_keys=podcast_data_keys,
...
class SongSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        song_data = pop_out_extra_fields(
            request_data=song_data,
            request_keys=song_data_keys,
            type_of_audio='song')
    ...
    def update(self, instance, validated_data):
        ...
        song_data = validated_data["data"]
        song_data_keys = list(song_data.keys())
        song_data = pop_out_extra_fields(request_data=song_data,
                                         request_keys=song_data_keys,
                                         type_of_audio='song')
